...
const SummaryCard = (props: ISummaryCardProps) => {
    if (props) {
        const { cardStyle, cardHeaderTitle, cardHeaderContent, cardBodyType, cardBodyContent } = props;
        return (
            <div className={cardStyle}>
                ...
                <div className='p-4'>
                    <SummaryContent cardBodyType={cardBodyType} cardBodyContent={cardBodyContent} />
                </div>
            </div>
        );
    } else return null;
...
const SummaryContent = ({ cardBodyType, cardBodyContent }: { cardBodyType: number, cardBodyContent: any }) => {
    if (cardBodyType === cardBodyTypes.NUMBER) {
        ...
        return (
            <h1 title={cardBodyContent} className='text-sm h-20 overflow-hidden w-full font-bold'>
                {cardBodyContent}
            </h1>
        );
